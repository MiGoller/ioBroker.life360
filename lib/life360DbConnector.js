...
async function setStateValue(dpId, dpName, dpRead, dpWrite, dpType, dpRole, val, ack) {
    myLogger.debug(`setStateValue --> ${dpId} = ${val}`);
...
async function setStateReadOnlyValue(dpId, dpName, dpType, dpRole, val, ack) {
    // myLogger.debug(`setStateReadOnlyValue --> ${dpId} = ${val}`);

    return setStateValue(dpId, dpName, true, false, dpType, dpRole, val, ack);

    // //  Create ioBroker state object
    // const obj = createStateReadOnlyDP(dpId, dpName, dpType, dpRole);
...
// function getObjPerson(circleId, member) {
//     const obj = {
//         "id": member.id,
//         "circleId": circleId,
...
//         "device": {
//             "battery": Number(member.location.battery),
//             "wifiState": Number(member.location.wifiState),
//             "charge": Boolean(member.location.charge)
//         },
//         "location": {
//             "latitude": Number(member.location.latitude),
//             "longitude": Number(member.location.longitude),
//             "accuracy": Number(member.location.accuracy),
//             "startTimestamp": Number(member.location.startTimestamp),
//             "endTimestamp": Number(member.location.endTimestamp),
//             "speed": Number(member.location.speed)
//         },
//         "place": {
...
function publishPlaces(places) {
    for (let p in places) {
        try {
            ...
            myLogger.debug(`Created / updated place ${places[p].id} --> ${places[p].name}`);
        } catch (error) {
...
// async function createPlaceDP(place) {
//     const idDP = `${dpPrefix.places}.${place.id}`;
//     const obj = {
//         "_id": idDP,
//         "type": "device",
//         "common": {
...
//     await adapter.setObjectNotExistsAsync(idDP, obj);
...
async function createPlaceStateDP(idDP, state, val) {
    ...
    // //  Create ioBroker object if it does not exists
    // await adapter.setObjectNotExistsAsync(dpID_state, obj);

    // //  Update state
    // await adapter.setStateAsync(dpID_state, val, true);
...
function publishPeople(persons) {
    for (let p in persons) {
        try {
            ...
            myLogger.debug(`Created / updated person ${persons[p].id} --> ${persons[p].name}`);
        } catch (error) {
            myLogger.error(error);
        }
    }
...
//     return `${dpPrefix.circles}.${circleId}`;
// }
...
//     return new Promise((resolve, reject) => {
//         const obj = {
//             "_id": `${dpPrefix.circles}.${circle.id}`,
//             "type": "device",
//             "common": {
//                 "name": circle.name,
...
//         adapter.setObjectNotExists(`${dpPrefix.circles}.${circle.id}`, obj, function(err) {
//             if (!err)
//                 resolve(obj);
//             else
//                 reject(err);
